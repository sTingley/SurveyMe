			...
			resolve(
				MongoClient.connect(`mongodb+srv://${application.config.dbUser}:${application.config.dbPass}@cluster0-rmf3a.mongodb.net/test?retryWrites=true&w=majority`,
					...
					(err, client) => {
						...
						routes.expose(application, db) // routes contains our endpoints
							.then(() => {
...
const start = async (application) => {
	...
	try {
		await Promise.all([authenticated, promiseDB])
			...
			.catch((err) => {
				application.logger.error('Should not have gotten here... You broke it...');
				throw err;
			})
...
const IngressService = () => {
	...
	// note this code could be moved into start / elsewhere as we break into microservices
